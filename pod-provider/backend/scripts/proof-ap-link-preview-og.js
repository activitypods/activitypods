...

{
  ...
  assert.strictEqual(extractFirstPreviewUrl(note2b), 'https://example.com/md-article', 'URL from Markdown source.content');
  ...
  assert.strictEqual(extractFirstPreviewUrl(note3), 'https://external.example/page', 'URL from content HTML <a href>');

  // Skip hashtag links
  const note4 = {
    type: 'Note',
    source: { mediaType: 'text/plain', content: 'Tagged #rust https://pods.example/tags/rust and https://real.com/page' },
  };
  ...
  const note5 = {
    type: 'Note',
    content: '<a href="https://mastodon.social/users/alice">@alice</a> see <a href="https://news.example/story">this</a>',
  };
...

{
  ...
  assert.ok(mergedArr.some(a => a.type === 'Image'), 'existing Image preserved');
  assert.ok(mergedArr.some(a => a.type === 'Link'), 'new Link added');
...

{
  ...
  try {
    ...
    const dummyNext = async ctx => { capturedParams = ctx.params; return 'ok'; };
    ...
    assert.deepStrictEqual(att.icon, { type: 'Image', url: 'https://news.example/thumb.jpg' }, 'OG image in attachment.icon');
    assert.strictEqual(capturedParams.collectionUri, 'https://pods.test/users/alice/outbox', 'collectionUri preserved');
