...
declare global {
  namespace Moleculer {
    ...
    /** TODO? */
    type ActionHandler<
      AllActions extends Record<string, ActionSchema> = AvailableActions,
      AllEvents extends Record<string, MoleculerEvent> = AvailableEvents,
      Ret extends ReturnType<AllActions[keyof AllActions & string]['handler']> = never
    > = ((ctx: Context<never, never, never, AllActions, AllEvents>) => Promise<Ret> | Ret) & ThisType<Service>;

    type ActionParamSchema = { [key: string]: any };

    type ActionParams = ValidationSchema;
    ...
    > = {
      ...
      handler?: ActionHandler<ActionsMapping, EventsMapping>;
    ...
    type ServiceActionsSchema<AllActions extends Record<string, ActionSchema> = AvailableActions> = {
      [key: string]: ActionHandler<AllActions> | ActionSchema<AllActions> | boolean;
    };
    ...
    /** Makes sure the functions always return promises. */
    ...
    /** TODO: Units all overloads for the function */
    type Call<
      AllActions extends ActionsMappingBase = AvailableActions,
      CActions extends CallableActions<AllActions> = CallableActions<AllActions>
    ...
    class Context<
      ...
      AllEvents extends Record<string, MoleculerEvent> = AvailableEvents
    > {
      constructor(broker: ServiceBroker<AllActions, AllEvents>, endpoint: Endpoint);

      id: string;
      broker: ServiceBroker<AllActions, AllEvents>;
      ...
      eventName: (string & {}) | keyof AllEvents | null;
      ...
      call: LegacyCall | Call<AllActions>;
      emit: LegacyEmit | Emit<AllEvents>;
      broadcast: LegacyEmit | Emit<AllEvents>;
    ...
    // TODO: Types used to represent the mapping and its components
    ...
    > {
      ...
      metadata?: any;
      actions?: ServiceActionsSchema<AllActions, AllEvents>;
      mixins?: Partial<ServiceSchema>[];
      methods?: ServiceMethods;
      hooks?: ServiceHooks;

      events?: ServiceEvents<AllEvents>;
    ...
    interface AvailableServices {
      'pre-service': { name: 'pre-service'; actions: { preAction: { params: {}; handler: () => {} } } };
    }

    /** All available actions, inferred from @see {AvailableServices}, mapped by their full name ("serviceName.actionName": Action). */
    type AvailableActions<Services extends Record<string, ServiceSchema> = AvailableServices> = {
      [S in keyof Services & string]: {
        [A in keyof Services[S]['actions'] & string as `${Services[S]['name']}.${A}`]: Services[S]['actions'][A];
      };
    }[keyof Services & string];

    type MoleculerEvent = any;
    interface AvailableEvents extends Record<string, MoleculerEvent> {}

    /** TODO? */
    class Service<
      SettingSchema extends ServiceSettingSchema | undefined = undefined, // Why was it `= ServiceSettingSchema` before
      AllActions extends Record<string, ActionSchema> = AvailableActions,
      AllEvents extends Record<string, MoleculerEvent> = AvailableEvents
    > implements ServiceSchema
    {
      constructor(
        broker: ServiceBroker<AllActions, AllEvents>,
        schema?: ServiceSchema<SettingSchema, AllActions, AllEvents>
      );

      protected parseServiceSchema(schema: ServiceSchema<SettingSchema, AllActions, AllEvents>): void;
      ...
      dependencies: string | ServiceDependency | (string | ServiceDependency)[];
      schema: ServiceSchema<SettingSchema, AllActions, AllEvents>;
      originalSchema: ServiceSchema<SettingSchema, AllActions, AllEvents>;
      broker: ServiceBroker<AllActions, AllEvents>;
      logger: LoggerInstance;
    ...
    type BrokerErrorHandler = (this: ServiceBroker, err: Error, info: BrokerErrorHandlerInfo) => void;
    ...
    > {
      ...
      call: LegacyCall | Call<ActionsMapping>;
      mcall<T>(
      ...
      emit: LegacyEmit | Emit<AllEvents>;
      broadcast: LegacyEmit | Emit<AllEvents>;
      broadcastLocal: LegacyEmit | Emit<AllEvents>;
